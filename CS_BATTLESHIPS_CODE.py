...
class FilePaths:
    def __init__(self):
        ...
        self.ocean_bg = r"PICTURES\mainMenu_bg_final.png"
...
def create_mainMenu(baseConfigs, filePaths, virtual_screen):
    ...
    return BUTTONS, COVER_IMAGE, RENDERED_TEXT, MENU_BG
...
def draw_tilemap_battleShipGame(baseConfigs, rendered_letters, rendered_numbers, GRID, virtual_screen):
    ...
    pygame.draw.rect(virtual_screen, (0, 0, 0), (baseConfigs.BOARD_X_ORIGIN - (baseConfigs.LINE_WIDTH_X % baseConfigs.TILE_SIZE),\
...
def draw_mainMenu(baseConfigs , buttons, cover_image, virtual_screen, highlighted_text):
...
def buttonLogic_mainMenu(baseConfigs, buttonChosen): # return the game state
...
def place_ship(baseConfigs, rendered_items, ship_object, start_pos, place_ship = True): # THE GRIDS ORIGIN (upper leftmost tile IS ALWAYS (1, 1) AND LAST POINT IS (GAMEBOARD_SIZE - 1, GAMEBOARD_SIZE - 1))
    ...
    for coord in ship_coords_to_be: # iterate through each coord the ship will be on
        ...
        if rendered_items[2][coord[1]][coord[0]].ship != None:
            ...
            rendered_items[2][coord[1]][coord[0]].colour = (255, 0, 0)
            place_ship = False # theres probably a better way to do this but it works rn
...
# scaling resolutions from the main resolution (1920, 1080) or wtv is in the virtual screen base config to the user chosen resolution
...
def battleShipsGame(baseConfigs, events, rendered_items, window, virtual_screen):
    ...
    virtual_screen.fill(baseConfigs.SCREEN_COLOUR) # background colour
    draw_tilemap_battleShipGame(baseConfigs, rendered_items[0], rendered_items[1], rendered_items[2], virtual_screen) # draw the tilemap to screen
    ...
    #highlight_selected_square(baseConfigs, rendered_items, virtual_screen) highlights an individual square the mouse is hovering on


    virtualToReal_window(baseConfigs, virtual_screen, window) # scale the window to user resolution
...
def mainMenu(baseConfigs, filePaths, events, virtual_screen, BUTTONS, COVER_IMAGE, window, rendered_text, menu_bg):
...
# screen manager using states


def displayed_screen(baseConfigs, filePaths, virtual_screen, BUTTONS, COVER_IMAGE, window, rendered_text, rendered_items, menu_bg): 
    ...
    while running:
        ...
        match game_state:
            ...
            case "mainmenu":
                ...
                game_state = mainMenu(baseConfigs, filePaths, events, virtual_screen, BUTTONS, COVER_IMAGE, window, rendered_text, menu_bg)
            
            case "battleships_game":

                game_state = battleShipsGame(baseConfigs, events, rendered_items, window, virtual_screen)
            
            case "settings":

                print("setting function here")
...
# starts up the code and stuff (creates the initial settings make it read from a setting file later)
...
def loadStartUp():
    ...
    BUTTONS, COVER_IMAGE, RENDERED_TEXT, MENU_BG = create_mainMenu(baseConfigs, filePaths, virtual_screen)
    ...
    displayed_screen(baseConfigs, filePaths, virtual_screen, BUTTONS, COVER_IMAGE, window, RENDERED_TEXT, RENDERED_ITEMS, MENU_BG)
